...
<div class="layers">
<div class="layer l1">Presentation <span class="desc">화면(StatefulWidget) · lib/presentation/screens/ (9개 화면)</span></div>
<div class="layer-arrow">↓ 직접 호출</div>
<div class="layer l2">Domain <span class="desc">TripPlan · DayPlan · PlaceItem (외부 의존 없음)</span></div>
<div class="layer-arrow">↓</div>
<div class="layer l3">Data <span class="desc">GeminiService · WeatherService · TripRepository · AuthService</span></div>
</div>

<br>

- **Application 레이어 없음** — 화면이 Service를 직접 호출 (소규모 7주 프로젝트에 맞춘 단순화)
- 각 Service는 **static 메서드**로 구성, 인스턴스 불필요
- GoRouter 8개 경로로 화면 전환 (`/`, `/home`, `/route/input`, `/route/result`, `/route/map`, `/weather`, `/mypage` ...)

<!--
[구현방법-아키텍처 / 1:20~1:45]
아키텍처는 3계층입니다. Presentation은 9개 화면, Domain은 TripPlan/DayPlan/PlaceItem 모델,
Data 레이어는 GeminiService, WeatherService, TripRepository, AuthService로 구성됩니다.
원래 계획에는 Application 레이어가 있었지만, 화면이 서비스를 직접 호출하는 구조로 단순화했습니다.
GoRouter로 8개 경로를 관리하고, 각 서비스는 static 메서드로 구성해서 인스턴스 생성 없이 바로 호출합니다.
-->
