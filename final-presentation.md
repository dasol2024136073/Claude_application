...
## <span class="chnum">02</span> 구현 방법 — 핵심 기능 4가지
...
> 네 기능 모두 **API 키 2개(Gemini, OpenWeatherMap)** + Unsplash(선택) 만으로 동작 — 발표 데모 환경 이식성 확보
...
셋째, 실시간 날씨 반영 — OpenWeatherMap에서 60개 이상 도시를 매핑하고, 실내외 카테고리가 바뀌면 재추천 알림을 보냅니다.
넷째, 지도 시각화 — flutter_map과 OpenStreetMap으로 API 키 없이 번호 마커와 폴리라인으로 동선을 보여줍니다.
이 네 기능 모두 API 키 1~2개만 있으면 동작해서, 데모 환경을 옮기기 쉽습니다.
-->
...
<div class="icon">🌦️</div>
<h3>날씨 변화 감지</h3>
운영 주기는 30분이지만, 데모 환경에서는 <strong>"5초 후 즉시 1회 체크"</strong>를 추가해 빠르게 결과 확인 가능
</div>
</div>
...
둘째 날씨 변화 감지는 원래 30분 주기지만 데모에서는 5초 후 즉시 한 번 체크하도록 추가했습니다.
-->
...
<div class="icon">✨</div>
<h3>Tripia 사용</h3>
취향+실시간 날씨를 반영해 10초 내 일정 생성, 날씨가 바뀌면 실내 코스로 자동 재추천
</div>
</div>
